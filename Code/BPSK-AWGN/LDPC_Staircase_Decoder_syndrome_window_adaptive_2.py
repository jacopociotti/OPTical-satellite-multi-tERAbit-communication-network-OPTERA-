...
class LDPC_Staircase_Decoder:
    ...
    def decode_sequence(self, received_llrs, original_bits = None, num_termination_blocks=2):
        ...
        total_decode_start_time = time.perf_counter()  # Inizio del timer per l'intera sequenza
        ...
        B_acc = [
            np.zeros((self.m, self.m), dtype=np.uint8)
        ...
        for i in range(1, num_info_blocks + 1):
            ...
            logger.info(f"NEW -Inizio Decodifica di B_{i}...")
            ...
            while True:
                ...
                for j in range(i, window_end):
                    ...
                    # Stessa identica decodifica delle m righe di A_j. Il ciclo
                    # sulle righe viene soltanto compilato/parallelizzato con Numba;
                    # sindrome, sweep, incremento di w e criteri decisionali restano
                    # nel medesimo ordine del decoder originale.
                    if hasattr(self.decode_row, "decode_component_adaptive"):
                        conv_batch, ext_batch = self.decode_row.decode_component_adaptive(
                            self.L_I[j-1],
                            self.L_E_left[j-1],
                            self.L_E_right[j-1],
                            self.L_I[j],
                        ...
                        component_decoder_calls += self.m
                        total_rows += self.m
                        ...
                        for k in range(self.m):
                            row_llr_in_sys[:self.m] = L_apriori_prev[:, k]
                            row_llr_in_sys[self.m:] = L_apriori_curr[k, :]
                ...
                B_curr = (L_tot_final_i < 0).astype(np.uint8)
                    
                # Prende la hard decision del blocco (i-1) già accettata dal decoder
                B_prev_acc = B_acc[i-1]
                    
                # Sparse Numba syndrome check when supported by the component decoder.
                if hasattr(self.decode_row, "count_staircase_syndrome"):
                    u_i = self.decode_row.count_staircase_syndrome(
                        B_prev_acc, B_curr, self.inv_perm
                    )
                else:
                    u_i = 0
                    for k in range(self.m):
                        row_sys = np.concatenate((B_prev_acc[:, k], B_curr[k, :]))
                        row_raw = row_sys[self.inv_perm] if self.perm is not None else row_sys
        ...
        if bler is not None:
            ...
            logger.info(f"BLER (Block Error Rate): N/A ({wrong_blocks}/{processed_blocks})")
