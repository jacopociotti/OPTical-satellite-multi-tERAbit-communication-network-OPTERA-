    ...
    def decode_sequence(self, received_llrs, original_bits = None, num_termination_blocks=2):
        ...
        for i in range(1, num_info_blocks + 1):
            ...
            while True:
                ...
                # Ottieni le hard decision per B_i usando L_I e L_E correnti
                L_tot_final_i = self.L_I[i] + self.L_E[i]
                B_curr = (L_tot_final_i < 0).astype(int)
                    
                # Prendi la hard decision del blocco (i-1) già accettata dal decoder
                ...
                for k in range(self.m):
                    ...
                    #s_k = np.bitwise_and(np.dot(row_raw, self.H_matrix), 1)
                    #s_k = np.dot(row_raw, self.H_matrix.T) % 2
                ...
                if u_i == 0:
                    ...
                    B_acc[i] = B_curr
                    block_accepted = True
                    #logger.info(f"Blocco {i} ACCETTATO. w={w_cur}, sweeps_totali={total_sweeps}, seq_sindromi={syndromes_ui}")
